...
func TestWFMFileDecoder_DecodeHeader_Valid(t *testing.T) {
	...
	// Magic header "WFM3"
	buffer.Write([]byte("WFM3"))
	
	// Padding
	binary.Write(&buffer, binary.LittleEndian, uint32(0))
	
	// DialoguePointerTable
	binary.Write(&buffer, binary.LittleEndian, uint32(0x1000))
	
	// TotalDialogues
	binary.Write(&buffer, binary.LittleEndian, uint16(50))
	
	// TotalGlyphs
	binary.Write(&buffer, binary.LittleEndian, uint16(200))
	...
	if string(header.Magic[:]) != "WFM3" {
		t.Errorf("Magic = %q, want %q", string(header.Magic[:]), "WFM3")
	}
...
func TestWFMFileDecoder_Decode_Complete(t *testing.T) {
	...
	// Check header
	if string(wfm.Header.Magic[:]) != "WFM3" {
		t.Errorf("Header.Magic = %q, want %q", string(wfm.Header.Magic[:]), "WFM3")
	}
